  ...
  **/     
  ui.addKeyboardsToMap = function()
  {
    ...
    for(var i in ui.regions)
    {
      ...
      // Get number of keyboards for each language
      languageCode='';
      for(var j=0; j<Keyboards.length; j++)
      { 
        if(Keyboards[j]['RegionCode'] != i) continue;      // Not this region
   ...
   * @param       {string}  PKeyboardName
   * @param       {string}  PLangCode              
   * @param       {string}  PRegCode              
   * Description  Rebuild the entire keyboard list whenever a keyboard is installed (without necessarily loading the keyboard)
   **/    
  ui.registerKeyboard = function(PInternalName,PLanguage,PKeyboardName,PLangCode,PRegCode)
  {
    ui.updateMap = true; 
    if(ui.startTimer) clearTimeout(ui.startTimer);
    ui.startTimer = setTimeout(ui.addKeyboardsToMap,2000);
